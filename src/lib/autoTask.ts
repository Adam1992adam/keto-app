// src/lib/autoTask.ts
// Auto-complete a daily task when the user performs the corresponding action.
// Also provides checkAchievements() for fire-and-forget achievement unlocks.
// Both silently fail — never block the main response.
...
/**
 * Check and award any newly unlocked achievements for a user.
 * Idempotent and silent — safe to call after any XP-awarding event.
 */
export async function checkAchievements(userId: string, accessToken: string): Promise<void> {
  try {
    ...
    const { data: existing } = await db
      .from('achievements').select('achievement_type').eq('user_id', userId);
...
/**
 * Mark a daily task as complete and award its XP.

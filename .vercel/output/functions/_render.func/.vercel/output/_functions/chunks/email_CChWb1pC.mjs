...
function getResend() {
  const key = "re_NZTQFWJS_AXQo4H1a6YYDuQvwsRZtxTY8";
  return new Resend(key);
}
const FROM = "Keto Journey <onboarding@resend.dev>";
const APP_URL = "https://keto-app-iota.vercel.app";
...
async function sendWelcomeEmail(to, name, tier) {
...
function step(num, icon, title, desc, link) {
...
async function sendWeeklySummaryEmail(to, name, stats) {
  const firstName = name.split(" ")[0];
...
async function sendMilestoneEmail(to, name, day, streak, totalXp) {
  const firstName = name.split(" ")[0];
  const milestones = {
...
async function sendStreakWarningEmail(to, name, streak) {
  const firstName = name.split(" ")[0];
  const content = `
    <!-- Warning header -->
...
async function sendWinbackEmail(to, name, daysMissed, currentDay) {
  const firstName = name.split(" ")[0];
  const content = `
    <div style="text-align:center;margin-bottom:24px;">
      <div style="font-size:48px;">👋</div>
...
export { sendMilestoneEmail, sendStreakWarningEmail, sendWeeklySummaryEmail, sendWelcomeEmail, sendWinbackEmail };
